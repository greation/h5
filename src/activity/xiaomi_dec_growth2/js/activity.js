...
vm.index = function () {
	new Vue({
		...
		data: {
			...
			checkPhoneYes: false,
			isLogin: false,
			awardText: ['288元现金券', '爱奇艺VIP会员1个月', '喜马拉雅46喜点', '288现金券'],
		...
		created: function () {
			...
			if (this.userData.isLogin !== 'N') {
				if (this.userData.activityType !== 'XIAOMIGROUP_DEC_2') {
			...
			cjreward: function () {
				...
				if (!this.isLogin) {
					...
					$.ajax({
							...
							async: false,
							data: {
								activityTypeStr: 'XIAOMIGROUP_DEC_2'
							},
							xhrFields: {
								withCredentials: true
							}
			...
			linkLogot: function () {
				$.ajax({
					...
					.done(function (data) {
						var data = JSON.parse(data);
						if (data.code === '000') {
							window.location.href = '/src/activity/xiaomi_dec_growth2/index.html?random=' + parseInt(Math.random() * 100000);
						}
					})
			},
				...
				if (!this.isXieYi) {
					...
					$.ajax({
							...
							data: {
								...
								'token_id': tokenId,
								'activityType': 'XIAOMIGROUP_DEC_2'
							}
						})
						.done(function (data) {
							if (data.isSuccess) {
