	...
	new Vue({
		...
		created: function () {
			...
			if (this.userData.isLogin !== 'N') {
				if (this.userData.activityType !== 'XIAOMISPORTNOVTWO') {
		...
		methods: {
			...
			activeStatus: function () {
				...
				$.ajax({
					...
					data: {
						activityTypeStr: 'XIAOMISPORTNOVTWO'
					}
				}).done(function (data) {
				...
				$.ajax({
					url: Helper.basePath + '/activityTurntable/getLotteryRewardName.htm',
					type: 'POST',
					dataType: 'json',
					async: false,
					xhrFields: {
			...
			luckyPost: function () {
				...
				var luckdata;
				$.ajax({
						url: Helper.basePath + 'activityTurntable/doXiaoMiNovTwoLottery.htm',
						type: 'POST',
						dataType: 'json',
						async: false,
						xhrFields: {
			...
			linkLogot: function () {
				$.ajax({
					...
					.done(function (data) {
						var data = JSON.parse(data);
						if (data.code === '000') {
							window.location.href = '/src/activity/xiaomi_eleven_motion/index.html?random=' + parseInt(Math.random() * 100000);
						}
					})
			},
				...
				if (!this.isXieYi) {
					...
					$.ajax({
							...
							data: {
								...
								'token_id': tokenId,
								'activityType': 'XIAOMISPORTNOVTWO'
							}
						})
						.done(function (data) {
							if (data.isSuccess) {
