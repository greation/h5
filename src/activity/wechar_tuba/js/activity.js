	...
	new Vue({
		...
		created: function () {
			...
			if (this.userData.isLogin !== 'N') {
				if (this.userData.activityType !== 'WECHAT_TUBA_ACTIVITY') {
		...
		methods: {
			...
			activeStatus: function () {
				...
				$.ajax({
					...
					data: {
						activityTypeStr: 'WECHAT_TUBA_ACTIVITY'
					}
				}).done(function (data) {
			...
			linkLogot: function () {
				$.ajax({
					...
					.done(function (data) {
						var data = JSON.parse(data);
						if (data.code === '000') {
							window.location.href = '/src/activity/wechar_tuba/index.html?random=' + parseInt(Math.random() * 100000);
						}
					})
			},
				...
				if (!this.isXieYi) {
					...
					$.ajax({
							...
							data: {
								...
								'token_id': tokenId,
								'activityType': 'WECHAT_TUBA_ACTIVITY'
							}
						})
						.done(function (data) {
							if (data.isSuccess) {
